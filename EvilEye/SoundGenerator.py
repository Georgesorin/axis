...
def generate_all():
    ...
    # 7. Trap (Buzzer grav și scurt pentru greșeală)
    trap1 = generate_tone(150, 0.1, vol=0.5, type='saw')
    trap2 = generate_tone(100, 0.3, vol=0.5, type='saw', slide=-50)
    save_wav("trap.wav", trap1 + trap2)

    # 8. Victory (Arpegiu fericit pentru câștigător)
    v1 = generate_tone(523.25, 0.2, vol=0.3, type='square')  # C5
    v2 = generate_tone(659.25, 0.2, vol=0.3, type='square')  # E5
    v3 = generate_tone(783.99, 0.2, vol=0.3, type='square')  # G5
    v4 = generate_tone(1046.50, 0.6, vol=0.3, type='square')  # C6
    save_wav("victory.wav", v1 + v2 + v3 + v4)
    ...
    # Countdown Beep (Scurt și ascuțit pentru 3, 2, 1)
    beep = generate_tone(880, 0.2, vol=0.4, type='square')
    save_wav("countdown_beep.wav", beep)

    # Start Whistle/GO!
    go_snd = generate_tone(1200, 0.6, vol=0.5, type='square', slide=-200)
    save_wav("countdown_go.wav", go_snd)
